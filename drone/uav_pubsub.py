...
class UAV:
	...
	#self.sitl_connection_str = "tcp:127.0.0.1:" + str(self.sitl_port)
        self.sitl_connection_str = "udp:127.0.0.1:" + str(self.sitl_port)
        self.vehicle = None
    ...
    def connect_sitl(self, connection_string, verbose):
        ...
        logging.info("%s Connecting to vehicle on: %s" % (self.prefix, connection_string))
	...
	logging.info(self.prefix + " Adding telemetry attribute callback to detect any change")
        return True

    def telemetry_remove_attr(self, vehicle, verbose):
        if verbose:
            print(self.prefix + " Removing telemtry attribute observer")
	logging.info(self.prefix + " Removing telemtry attribute observer")
    ...
    def get_data(self, socket, verbose):
		...
		logging.info("GCS Current Timestamp: " + repr(time.time()))
